...
診断回答の保存はGoogle Apps Script経由でGoogleスプレッドシートに送信します。
認証、決済、専用DBは使用していません。
...
診断回答をGoogleスプレッドシートへ保存する場合は、GitHub ActionsのRepository secretに `GOOGLE_SHEETS_WEBHOOK_URL` を設定してください。詳しくは [GOOGLE_SHEETS_SETUP.md](GOOGLE_SHEETS_SETUP.md) を参照してください。
...
| --- | --- |
| `/` | トップページ |
| `/diagnosis` | 診断前の基本情報入力・利用規約同意ページ |
...
診断前に氏名、電話番号、メールアドレス、性別、利用規約への同意を取得します。
回答、入力情報、診断結果はGoogle Apps Script経由で指定のGoogleスプレッドシートに送信されます。
回答中の一時データと結果表示用データはブラウザの `localStorage` に保存され、診断結果画面の「もう一度診断する」で削除できます。
...
- `src/lib/diagnosis/submission.ts`: Google Sheets送信用payload作成と送信処理
